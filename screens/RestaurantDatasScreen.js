...
class RestaurantDatasScreen extends Component {
  ...
  getRestaurantDatas() {
    ...
    if (this.props.user_data.target_type === 'Restaurant') {
      fetch(
        configjson.base_url + 'restaurants/' + this.props.user_data.target_id,
        {
          headers: {
        ...
        response.json().then(responseJson => {
          ...
          console.log(
            '-----getRestaurantDatas() target type Restaurant----------',
            res,
          );
      ...
      console.log(
        'target type NEM EGYENLO restaurants',
        this.props.user_data.restaurants,
      );
    ...
    if (this.state.income_response) {
      ...
      return (
        <>
          ...
          <View
            style={{width: '85%', flexDirection: 'row', alignSelf: 'center'}}>
            <Text
              style={{
                alignSelf: 'flex-start',
                fontSize: 18,
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    setSelectingMode: selectingMode =>
      dispatch(actionHelper.setSelectingMode(selectingMode)),
    setSelectedRestaurant: selectedRestaurant =>
      dispatch(actionHelper.setSelectedRestaurant(selectedRestaurant)),
    setFreshOrders: fresh_orders =>
      dispatch(actionHelper.setFreshOrders(fresh_orders)),
  };
};

export default withNavigation(
  connect(mapStateToProps, mapDispatchToProps)(RestaurantDatasScreen),
);
