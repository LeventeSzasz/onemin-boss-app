...
let segmentData = [
  ...
  },
  {
    ...
    value: 'platforms',
    arrayHelper: 'platforms',
    itemName: 'platform',
  },
  {
    label: Localized.product_categories,
    value: 'product-categories',
    arrayHelper: 'product_categories',
    itemName: 'product_category',
  },
  {
    label: Localized.products,
    value: 'products',
    arrayHelper: 'products',
    itemName: 'product',
  },
  {
    label: Localized.subproducts,
    value: 'subproducts',
    arrayHelper: 'subproducts',
    itemName: 'subproduct',
  },
  {
    label: Localized.options,
    value: 'options',
    arrayHelper: 'options',
    itemName: 'option',
  },
  {
    ...
    value: 'shadow-customers',
    arrayHelper: 'shadow_customers',
    itemName: 'shadow_customer',
  },
];
...
class StatisticsScreen extends React.PureComponent {
  ...
  renderChart(segment_by, statistics) {
    ...
    if (
      segment_by.value == 'product-categories' ||
      segment_by.value == 'products' ||
      segment_by.value == 'subproducts' ||
      segment_by.value == 'options'
    ) {
        ...
        <View style={{flexDirection: 'row', padding: 10}}>
          <View style={{marginTop: 10, width: '50%', marginBottom: 10}}>
            {statistics.map((item, index) => {
              ...
              return (
                <TouchableOpacity
                  style={{justifyContent: 'center', height: rowHeight}}
                  key={index}
                  onPress={
                    segment_by.value == 'product-categories'
                      ? () =>
                          this.showInfoModal(this.state.raw_statistics[index])
                      : () => console.log('nemkellmodal')
                  }>
                  <Text
                    key={index}
                    style={{
    ...
    if (
      segment_by.value == 'platforms' ||
      segment_by.value == 'payment-methods'
    ) {
                  ...
                  <View key={index} style={{flexDirection: 'row'}}>
                    ...
                    <Text
                      ...
                      <Text
                        ...
                        (
                        {Math.round(
                          (item.percent * 100 + Number.EPSILON) * 100,
                        ) / 100}
                        %)
                      </Text>{' '}
                      {item.key}:{' '}
  ...
  getV3Statistics(partner_id, restaurant_id, callback) {
    let dateString =
      'from=' +
      moment(new Date()).startOf('day').format('YYYY-MM-DD[T]HH:mm:ss[Z]') +
      '&till=' +
      moment(new Date()).endOf('day').format('YYYY-MM-DD[T]HH:mm:ss[Z]');
    ...
    fetch(URL, {
      ...
      .then(response => {
        ...
        response.json().then(responseJson => {
          ...
          if (
            this.props.segment_by.value == 'product-categories' ||
            this.props.segment_by.value == 'products' ||
            this.props.segment_by.value == 'subproducts' ||
            this.props.segment_by.value == 'options'
          ) {
            this.makeV3DataForBarChart(res);
          }
          if (
            this.props.segment_by.value == 'platforms' ||
            this.props.segment_by.value == 'payment-methods'
          ) {
  ...
  makeV3DataForPieSVG(raw_statistics, segment_by) {
    ...
    for (let key in helperData) {
      ...
      pieChartSVGData.push({
        ...
        key:
          TranslateSegmentFunction(key) != undefined
            ? TranslateSegmentFunction(key)
            : key,
      ...
      <SafeAreaView style={{backgroundColor: '#E0E7E9'}}>
        <ScrollView
          ...
          <TouchableOpacity
            ...
            }} /*onPress={ () => this.segmentPickerReference.togglePicker(true) } */
            onPress={() => this.showSegmentModal()}>
            <View style={{flex: 1, justifyContent: 'center'}}>
              <Text style={{color: 'black', fontFamily: configjson.boldFont}}>
                {Localized.segment}
              </Text>
              <Text style={{fontSize: 20, fontFamily: configjson.lightFont}}>
                {this.state.segment_by.label}
              </Text>
            </View>
            <Image
              style={{
              ...
              source={require('../resources/levels.png')}
              resizeMode="contain"
            />
          </TouchableOpacity>

          {this.state.segmentModalVisible ? (
            <SegmentModal
              isVisible={this.state.segmentModalVisible}
              closeModal={() => this.closeSegmentModal()}
              segments={segmentData}
              selectSegment={this.setSegmentBy.bind(this)}
            />
          ) : null}
          ...
          {this.renderChart(
            this.state.segment_by,
            this.state.statisticsSVG,
            this.state.loading,
          )}
        </ScrollView>
      </SafeAreaView>
